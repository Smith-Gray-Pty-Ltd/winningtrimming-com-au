...
import type { AllPillarSuburbData } from './matrix'
...
const PillarCard: React.FC<{
  ...
  regionSlug: string
  suburbSlug: string
}> = ({ pillar, image, regionSlug, suburbSlug }) => {
  return (
    <Link
      href={`/${pillar.slug}/${regionSlug}/${suburbSlug}`}
...
export const AllPillarSuburbTemplate: React.FC<{ data: AllPillarSuburbData }> = async ({ data }) => {
  const { region, suburb, nearbySuburbs } = data
  ...
  return (
    <div className="pb-24">
      ...
      ==================================================================== */}
      <section
        className="relative flex min-h-[55vh] items-center overflow-hidden text-white"
        data-theme="dark"
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            <div className="lg:col-span-2 max-w-2xl">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight leading-[1.1]">
                Trimming Services in {suburb.title}
              </h1>
              <p className="mt-6 text-lg text-white/75 leading-relaxed max-w-xl">
                Custom-made covers, canvas and upholstery — plus expert repairs. Serving{' '}
                {suburb.title}, {region.title} from our Toronto workshop on Lake Macquarie.
              </p>
            </div>
              ...
              <div className="rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 p-8 w-full lg:max-w-xs">
                ...
                <div className="pt-5 border-t border-white/20">
                  ...
                  <p className="text-sm leading-relaxed">
                    <Link href={`/${region.slug}`} className="font-medium text-white hover:text-white/70 transition-colors">
                      {region.title}
                    </Link>
                  </p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-3">
              <div className="prose dark:prose-invert max-w-3xl text-foreground/80 leading-relaxed">
                <p>
                  Winning Trimming provides custom and repair trimming services in {suburb.title}, {region.title}.
                  From marine canvas and boat upholstery to automotive seat re-trims, caravan cushions,
                  machinery covers and commercial upholstery — we handle every type of trimming work
                  from our workshop in Toronto on Lake Macquarie.
                </p>
                <p>
                  {suburb.title} is in the {region.title} area, a short drive from our Toronto workshop.
                  For larger jobs like marine clears and canvas we can come to your marina, mooring or
                  home to measure and fit on-site. For automotive and caravan work you can bring the
                  vehicle to our workshop.
                </p>
                <p>
                  We use premium marine-grade and automotive-grade materials — UV-stable vinyl,
                  Sunbrella canvas, WeatherMax and high-density foams — all stitched with UV-stable
                  thread to withstand the harsh Australian conditions. Call {PHONE} or request a quote online.
                </p>
              </div>
              <div className="mt-6">
                <CMSLink
      ...
      {/* ====================================================================
          4. SERVICE PILLARS — link to /{pillar}/{region}/{suburb}
      ==================================================================== */}
      <section className="py-16 bg-muted/30">
        <div className="container">
          <div className="mb-10 text-center">
            <h2 className="text-3xl font-medium tracking-tight">What We Do in {suburb.title}</h2>
            <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">
              Five service areas covering everything from boats to trucks. Explore each
              area to see what we offer in {suburb.title}, {region.title}.
            </p>
          </div>
          <div className="flex flex-wrap justify-center gap-6">
            {SERVICE_PILLARS.slice(0, 3).map((pillar) => (
              <PillarCard key={pillar.slug} pillar={pillar} image={pillarImages[pillar.slug]} regionSlug={region.slug || ''} suburbSlug={suburb.slug || ''} />
            ))}
          </div>
        ...
        <div className="container">
          <div className="rounded-2xl bg-[#607A00] text-white p-10 md:p-16 text-center">
            ...
            <p className="mt-3 text-white/85 max-w-2xl mx-auto">
              ...
              solution for your vessel, vehicle or business in {suburb.title}.
            </p>
